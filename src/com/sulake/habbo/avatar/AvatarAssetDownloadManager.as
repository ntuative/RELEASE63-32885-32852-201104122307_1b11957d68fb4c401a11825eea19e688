package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_937:int = 3;
      
      private var var_421:AssetLoaderStruct;
      
      private var var_935:Timer;
      ...
      private const const_1724:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_935 = new Timer(this.const_2050,1);
         this.var_935.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_421)
         {
            this.var_421.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_421.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_421)
         {
            this.var_421.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_421.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_937);
            if(this.var_1405.indexOf("?") > 0)
            {
               _loc2_ = this.var_1405 + "&retry=" + this.var_937;
            }
            else
            {
               _loc2_ = this.var_1405 + "?retry=" + this.var_937;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         for each(_loc2_ in param1.lib)
         {
            ...
            for each(_loc4_ in _loc2_.part)
            {
               ...
               _loc6_ = this.var_1146[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1146[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1729)
         {
            this.var_1145.push([param1,param2]);
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_745.length > 0 && this.var_744.length < this.const_1724)
         {
            _loc1_ = this.var_745[0];
            _loc1_.startDownloading();
            this.var_744.push(this.var_745.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_745.indexOf(param1) == -1 && this.var_744.indexOf(param1) == -1)
         {
