package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_785:ColorTransform;
      
      private var var_1472:ColorTransform;
      
      private var var_1825:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_840:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_785 = new ColorTransform();
         this.var_785.alphaMultiplier = 1;
         this.var_1472 = new ColorTransform();
         this.var_1825 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_97 = this.var_1184[param1];
         this.var_977 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1826 >= 0)
         {
            this.var_266 = this._visualization.getSprite(this.var_1826);
            if(this.var_266 && this.var_266.asset)
            {
               ...
               if(this.var_87 == null)
               {
                  this.var_87 = this.var_266.asset.clone();
                  if(this.var_785.alphaMultiplier != 1)
                  {
                     this.var_642 = new BitmapData(this.var_87.width,this.var_87.height,true,4278190080);
                  }
               }
               this.var_2300 = -this.var_266.offsetX;
               this.var_2299 = -this.var_266.offsetY;
               this.var_266.asset = this.var_87;
            }
            if(this.var_87)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_97.y * this.var_1183;
         }
         return 0;
      }
      ...
      {
         if(!this.var_97 || !this.var_266)
         {
            return;
         }
         if(this.var_87 && this.var_266.asset != this.var_87)
         {
            this.var_266.asset = this.var_87;
         }
         if(this.var_977)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_977 && this.var_97.hasIgnited)
         {
            this.var_977 = true;
         }
         _loc8_ = this.var_1105 * this.var_1183;
         this.var_97.update();
         if(this.var_977)
         {
            if(this.var_97.roomObjectSpriteId >= 0)
            ...
            {
               this.var_87.draw(this.var_642,this.var_1825,this.var_785,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_97.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2300 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1183;
               _loc4_ = this.var_2299 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1183;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1472.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_87.draw(_loc7_,this._translationMatrix,this.var_1472,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2606 = parseInt(param1.@size);
         this.var_1826 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1105 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1183 = this.var_2606 / 64;
         this.var_840 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_840 = Math.min(this.var_840,1);
         this.var_785.alphaMultiplier = this.var_840;
