package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1407:String = "asset-libraries";
      
      private static const const_1412:String = "library";
      
      private static const const_1411:String = "service-libraries";
      
      private static const const_1409:String = "library";
      
      private static const const_1404:String = "component-libraries";
      
      private static const const_1410:String = "library";
      
      private static const const_1405:String = "error_data";
      
      private static const const_1408:String = "error_cat";
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_897:
               ...
               var_404.addChild(new ProfilerViewer(this.var_207));
               break;
            case Core.const_509:
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1405,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1408,String(param3));
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         try
         {
            node = config.child(const_1407)[0];
            if(node != null)
            {
               list = node.child(const_1412);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_314,e);
         }
         try
         {
            node = config.child(const_1411)[0];
            if(node != null)
            {
               list = node.child(const_1409);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_314,e);
         }
         try
         {
            node = config.child(const_1404)[0];
            if(node != null)
            {
               list = node.child(const_1410);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_314,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1544);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_815)
                  {
                     this.var_815.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_534 - this.var_382.length,this.var_534,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_207 ? 0 : this.var_1517 & 0;
         if(_loc3_ == Core.const_509)
         {
