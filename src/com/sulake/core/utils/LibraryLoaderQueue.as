package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1724:int = 4;
      ...
      {
         return this.var_422.length + this.var_288.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_288.length < const_1724 && this.var_422.length > 0)
            {
               _loc1_ = this.var_422.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_422.indexOf(loader);
            if(index > -1)
            {
               this.var_422.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_422)
            {
               if(param2)
               {
