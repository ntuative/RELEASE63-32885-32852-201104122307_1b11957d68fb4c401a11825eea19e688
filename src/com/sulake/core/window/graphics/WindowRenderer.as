   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(this.getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            this.var_293.x = param2.x - param1.x;
            this.var_293.y = param2.y - param1.y;
            this.var_293.width = param2.width;
            this.var_293.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_33))
            {
               while(param1.testParamFlag(WindowParam.const_33))
               {
                  ...
                  this.var_293.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            this.getWindowRendererItem(param1).invalidate(param1,this.var_293,WindowRedrawFlag.const_1134);
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            ...
            param1.addEventListener(WindowDisposeEvent.const_864,this.windowDisposedCallback);
         }
      }
      
      public function removeRenderable(param1:IWindow) : void
      {
         param1.removeEventListener(WindowDisposeEvent.const_864,this.windowDisposedCallback);
         var _loc2_:WindowRendererItem = this.var_345[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            ...
            if(!_loc1_.disposed)
            {
               ...
               this.var_293.x = _loc3_.dirty.x;
               this.var_293.y = _loc3_.dirty.y;
               this.var_293.width = _loc3_.dirty.width;
               this.var_293.height = _loc3_.dirty.height;
               this.renderWindowBranch(_loc1_,this.var_293,_loc1_.rectangle.clone());
            }
            _loc4_++;
         }
      }
      ...
      private function renderWindowBranch(param1:WindowController, param2:Rectangle, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param2,this.var_1631,this.var_1354))
            {
               if(this.getWindowRendererItem(param1).render(param1,this.var_1631,this.var_1354))
               {
